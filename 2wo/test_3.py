...
image_main = cv.imread("test\\2_angle_271.jpg")
...
contours = cv.findContours(image_first.copy(), cv.RETR_TREE, cv.CHAIN_APPROX_TC89_KCOS)[
    0
]
for cnt in contours:
...
) > math.sqrt((cnts1[0][0] - cnts1[3][0]) ** 2 + (cnts1[0][1] - cnts1[3][1]) ** 2):
    # #неч
